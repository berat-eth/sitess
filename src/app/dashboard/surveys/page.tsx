...
export default function SurveysPage() {
  ...
  const surveyTemplates: SurveyTemplate[] = useMemo(() => [
    {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {surveyTemplates.map((template) => (
                <motion.div
                  ...
                  onClick={() => {
                    const selectedTemplate = surveyTemplates.find(t => t.id === template.id);
                    if (selectedTemplate) {
                      setSurvey(prev => ({
                        ...prev,
                        title: selectedTemplate.name,
                        description: selectedTemplate.description,
                        questions: selectedTemplate.questions.map((q: any) => ({ ...q, id: Date.now().toString() + Math.random() })),
