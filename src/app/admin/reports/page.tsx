...
const AdminReportRow = ({ report, index }: { report: Report; index: number }) => {
        ...
        <div className="flex items-center gap-2 justify-end">
          {report.status === 'ready' ? (
            <>
              ...
              <button className="text-green-600 hover:text-green-900 transition-colors duration-200" title="Müşteriye Gönder">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="space-y-8">
      ...
      >
        <div>
          ...
          <p className="text-gray-600 mt-1">Sistem raporlarını yönetin ve müşterilere gönderin</p>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredReports.map((report, index) => (
                <AdminReportRow key={report.id} report={report} index={index} />
              ))}
            </tbody>
          </table>
        </div>
